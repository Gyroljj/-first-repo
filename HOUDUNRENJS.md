...
假如有这样的场景，如果我们想通过正则查找`/`符号，但是 `/`在正则中有特殊的意义。如果写成`///`
这会造成解析错误，所以要使用转义语法 `/\//`来匹配。
...
每个字符都有属性，如`L`属性表示是字母，`P` 表示标点符号，需要结合 `u`
模式才有效。其他属性简写可以访问 [属性的别名](https://www.unicode.org/Public/UCD/latest/ucd/PropertyValueAliases.txt)
网站查看。
...
字符也有 unicode 文字系统属性 `Script=文字系统`，下面是使用 `\p{sc=Han}` 获取中文字符 `han`
为中文系统，其他语言请查看[文字语言表](https://www.unicode.org/standard/supported.html)
...
(JavaScript/regexp/23.html)是使用原子表知识删除所有标题
...
把所有`教育`汉字加上链接 `https://www.houdunren.com`  (JavaScript/regexp/40.html)

为链接添加上`https` ，并补全 `www.`  (JavaScript/regexp/41.html)
...
判断是否正确，检测输入的邮箱是否合法  [示例](JavaScript/regexp/44.html)
...
计算内容中后盾人出现的次数  [示例](JavaScript/regexp/45.html)
...
把后面是`教程` 的后盾人汉字加上链接  [示例](JavaScript/regexp/46.html)

将价格后面 添加上 `.00`  [示例](JavaScript/regexp/47.html)

### (?<=exp)

**零宽后行断言** `?<=exp` 匹配前面为 `exp` 的内容

所有超链接替换为`houdunren.com`  [示例](JavaScript/regexp/48.html)

将电话的后四位模糊处理  [示例](JavaScript/regexp/49.html)

### (?!exp)

**零宽负向先行断言** 后面不能出现 `exp` 指定的内容

用户名中不能出现`向军`  [示例](JavaScript/regexp/51.html)

### (?<!exp)

**零宽负向后行断言** 前面不能出现 `exp` 指定的内容

获取前面不是数字的字符  [示例](JavaScript/regexp/52.html)

把所有不是以 `https://oss.houdunren.com` 开始的静态资源替换为新网址  [示例](JavaScript/regexp/53.html)

# Promise

`JavaScript` 中存在很多异步操作,`Promise`
将异步操作队列化，按照期望的顺序执行，返回符合预期的结果。可以通过链式调用多个 `Promise` 达到我们的目的。
...
[示例](JavaScript/promise/2.html) 是加载图片使用回调函数处理，代码嵌套较麻烦

### 定时嵌套

一个定时器执行结束后，执行另一个定时器，这种嵌套造成代码不易阅读  [示例](JavaScript/promise/3.html)

### 加载文件

异步加载外部`JS`文件，需要使用回调函数执行  [示例](JavaScript/promise/4.html)
...
使用传统的异步请求也会产生回调嵌套的问题，下面是获取向军的成绩，需要经过以下两步  [示例](JavaScript/promise/5.html)
